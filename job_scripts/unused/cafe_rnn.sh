...
for wd in 1e-3; do # tune
    ...
    for nmp in 1 2 ; do # tune
    for model in predrnn_v1; do # ????
        for noise in 0; do 
        for lr in 5e-4 ; do # tune
        for dx in 1; do
        for unet in 0 1; do # tune
        for nldec in 1; do
        for RNN in 1; do
            for loss in L2 ; do
            DIR=experiment/${dat}_RNN${model}-batch${batch}_lr${lr}_nin${n_in}_nout${n_out}_noise${noise}_loss${loss}_nmp${nmp}_nhid${nhid}_ker${ker}_RNN${RNN}_dx${dx}_unet${unet}_wd${wd}_rot${rot}_nldec${nldec}
            mkdir -p $DIR
            echo $DIR $device
 ...
 --dx=$dx --autoencoder=None+pw$nldec \
