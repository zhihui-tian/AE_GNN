...
from NPS.model.common import ConvND, my_activations



class irevnet_block(nn.Module):
    def __init__(self, in_ch, out_ch, stride=1, first=False, dropout_rate=0.,
                 affineBN=True, mult=2, dim=2, periodic=False):
...
class autoencoder(nn.Module):
    def __init__(self, nBlocks, nStrides, nChannels=None, init_ds=2,
                 dropout_rate=0., affineBN=True, in_shape=None, mult=2, dim=2, periodic=False):
...
class zig_rev_predictor(nn.Module):
    def __init__(self, input_size, hidden_size,output_size,n_layers,batch_size,temp =3, w =8,h = 8, dim=2, periodic=False):
        ...
        self.convlstm = nn.ModuleList(
                [STConvLSTMCell(input_size if i==0 else hidden_size, hidden_size, hidden_size, dim=dim, periodic=periodic) for i in
                 range(self.n_layers)])
