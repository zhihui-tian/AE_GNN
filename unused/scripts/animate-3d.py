...
parser.add_argument("--tstart", type=int, default=0, help="Max time")
parser.add_argument("--tend", type=int, default=sys.maxsize, help="Max time")
parser.add_argument("--nsamples", type=int, default=np.inf, help="Number of samples")
parser.add_argument("--noticks", action='store_true', help='Removes axis ticks from plot')
parser.add_argument("-o", default='', help="save as gif")
parser.add_argument("data", help="data file(s) (.npy or .npz)", nargs='+')
options = parser.parse_args()

nplot= min(len(options.data), options.nsamples)
print('nplot', nplot)
...
for i in range(nplot):
    load = load_array(options.data[i]).astype('float32')
    print('Plot {}, Load {}'.format(i, load.shape))
    data.append(load)
    ...
    print('a)', *[d.shape for d in data])
    data[i] = data[i][options.tstart:options.tend:options.tskip, ..., options.ichannel[i]]
    print('b)', *[d.shape for d in data])
...
def plot_3D_slice(array, ax, noticks=False):
    pic = []
    min_val = array.min()
    max_val = array.max()
    ...
    if noticks:
        print('Removing ticks from', ax)
        for line in ax.xaxis.get_ticklines(): line.set_visible(False)
        ax.xaxis.set_ticklabels([])
        for line in ax.yaxis.get_ticklines(): line.set_visible(False)
        ax.yaxis.set_ticklabels([])
        for line in ax.zaxis.get_ticklines(): line.set_visible(False)
...
def plot_3D_iso(arr, ax, noticks=False):
    try:
        from skimage import measure 
...
if options.type == 'slice':
    ...
    plotfunc=lambda t: [plot_3D_iso(data[i][t], axs[i], noticks=options.noticks) + plot_3D_slice(data[i][t], axs[i], noticks=options.noticks) for i in range(nplot)]
...
def animate(t):
    ...
    ltime = time.time()
    rtime = dtime * (nframe - t) / 60
    print('{}/{}. Time Taken: {:.2f} s. Est. Remaining: {:.2f} min'.format(t, nframe-1, dtime, rtime))
    return plots
...
if options.o:
    print('Saving to', options.o)
    anim.save(options.o, fps=6, dpi=300)
else:
    plt.show()
