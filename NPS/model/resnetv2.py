...
class resblock(nn.Module):
    def __init__(self, n_feat, kernel_size,
        bn=True, act=nn.ReLU(True), dim=2, periodic=False):
        super(resblock, self).__init__()
        self.act = act
        m = []
        for i in range(2):
            m.append(ConvND(n_feat, n_feat, kernel_size, dim=dim, periodic=periodic, bias=False))
            if bn: m.append(NNop[(dim,'bnorm')](n_feat))
#            if i == 0: m.append(act)
            if i==0 and act is not None: m.append(act)
