...
class Model(nn.Module):
    def __init__(self, args, ckp):
        ...
        if not args.cpu and args.n_GPUs > 1:
            ...
            proc_group = torch.distributed.init_process_group('mpi')
            self.model = nn.parallel.DistributedDataParallel(self.model, None, process_group=proc_group) 
    ...
    def load(self, apath, pre_train='', resume=-1, cpu=False):
        ...
        if resume == -1 or resume == -2:
            fname = os.path.join(apath, 'model_latest.pt' if resume==-1 else 'model_best.pt')
            if os.path.exists(fname):
                ...
                print(f'No latest found {fname}')
        elif resume == 0:
            if pre_train:
                print('Loading model from pretrained {}'.format(pre_train))
                self.get_model().load_state_dict(
                    torch.load(pre_train, **kwargs),
                    strict=False
                )
        else:
            print('Loading from numbered ', os.path.join(apath, 'model_{}.pt'.format(resume)))
            self.get_model().load_state_dict(
                torch.load(
