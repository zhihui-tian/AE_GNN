...
post_processors = []
for x in sys.argv[1:]:
    print(f'Registering commandline arguments from {x}')
    if x.startswith(('--register_args=', '--model=', '--datatype=', '--dataset=')):
...
if args.test_set == 'predict':
    ...
    args.n_out_test = 0 # no ground truth target in prediction mode
